...
const Carousel: React.FC<CarouselProps> = ({
  ...
  return (
    ...
    >
      <div className={cn(className, "overflow-hidden")} ref={emblaRef}>
        {children}
      </div>
    </CarouselContext.Provider>
  );
...
export const SliderProgress = ({ className }: { className?: string }) => {
  ...
  return (
    <div
      className={cn(
        "  0 relative rounded-md h-2 justify-end items-center w-96 max-w-[90%] overflow-hidden",
        className
      )}
    >
      <div
      ...
      <div className="flex gap-2">
        {scrollSnaps.map((_: any, index: React.Key | null | undefined) => (
          <button
            ...
            className={`relative inline-flex  p-0 m-0 w-2 h-2 `}
          >
            <div className=" 0/40  h-2 rounded-full w-10 "></div>
            {index === selectedIndex && (
              <AnimatePresence mode="wait">
                <motion.div
                  ...
                  className={cn(
                    "absolute z-[3] w-full h-full left-0 top-0 bg-stone rounded-full",
                    activeclass
                  )}
                />
              </AnimatePresence>
